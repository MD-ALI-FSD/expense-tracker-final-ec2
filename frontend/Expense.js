...
async function fetchData() {
  ...
  const config = {
   ...
   const datarv = await axios.get("http://43.205.59.61/user/getexpense", config)
  //  console.log(datarv);
...
async function displayData() {
  ...
  const userDetails = await axios.get("http://43.205.59.61/user/getuser", config);
...
submit.addEventListener("click", async function (e) {
  ...
  if (id === -2) {
    // storing new data
    axios
      .post("http://43.205.59.61/user/addexpense", newUserData, config)
      .then((res) => {
    ...
    axios
      .put(`http://43.205.59.61/user/editexpense/${id}`, newUserData)
      .then((res) => {
...
deleteUserBtn.addEventListener("click", async function(e) {
  ...
  if (confirmed) {
    ...
    const res = await axios.delete("http://43.205.59.61/user/delete-user", config);
...
downloadBtn.addEventListener("click", async function(e){
  ...
  const response  = await axios.get("http://43.205.59.61/download/expenses", config);
  // to download file
...
parent.addEventListener("click", async function editDelete(e) {
  ...
  if (e.target.className === "deletebtn") {
    ...
    axios
      .delete(`http://43.205.59.61/user/deleteexpense/${e.target.id}`, config)
        .then((res) => {
          // displayData();
...
rzpbtn.addEventListener("click", async function (e) {
  ...
  const datarv = await axios.get("http://43.205.59.61/user/purchasepremium", config);
  // console.log(datarv);
  ...
  let options = {
    ...
    handler: async function (resp) {
      await axios.post(
        "http://43.205.59.61/user/updatetransactionstatus",
        {
